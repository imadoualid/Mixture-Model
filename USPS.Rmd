---
title: "Projet Mixture model : USPS"
...
USPS ( US Post Office Zip Code ) est un dataset contenant des images de chiffres manuscrits de taile 16x16, pour un total donc de 256 variable représentant chacun un pixel noir et blanc de l'image.

```{r}
USPS = readMat(con="C:\\Users\\GIGABYTE\\Downloads\\DATA_MATLAB - Projet-master-MLDS\\DATA_MATLAB - Projet-master-MLDS\\USPS.mat")
```


```{r}
y_data = as.data.frame(USPS$y)
x_data = as.data.frame(USPS$X)
...
#  Q2: ACP
Depuis le graph des individus, on remarque que les classes sont trop mélangé et il est difficile de les séparer.Lorsque l'on représente uniquement les modalité de la variable cible y, on peut voir que les individu des classes y_2 et y_1 contribuent fortement à la creation de l'axe 1 alors que les individidus de la classes y_7 et y_8 contribuent à la creation de l'axe 2.

```{r}
res.PCA<-PCA(x_data,quali.sup=c(257),graph=FALSE)
plot.PCA(res.PCA,choix='var',title="Graphe des variables de l'ACP")
plot.PCA(res.PCA,invisible=c('ind.sup'),select='contrib  1035',habillage=257,title="Graphe des individus de l'ACP",label =c('quali'))
```
...
res.PCA<-PCA(x_data,quali.sup=c(257),graph=FALSE)
plot.PCA(res.PCA,choix='var',title="Graphe des variables de l'ACP")
plot.PCA(res.PCA,invisible=c('ind','ind.sup'),select='contrib  1035',habillage=257,title="Graphe des individus de l'ACP",label =c('quali'))
```
...
La plus part des algorithme ne trouvent que 2 classes en tant que nombre optimal lorsque nous appliquons un clustering, nous pouvons donc proposer 2 classes dans ce cas la, ce qui est tres loin du nombre de classe original.
```{r}
res_cah_ward <-NbClust(x_data[,1:256], distance = "euclidean", min.nc=2, max.nc=10, method = "ward.D2",
index = "ch")
res_cah_average<-NbClust(x_data[,1:256], distance = "euclidean", min.nc=2, max.nc=10, method = "average",
index = "ch")
res_cah_complete <-NbClust(x_data[,1:256], distance = "euclidean", min.nc=2, max.nc=10, method = "complete",
index = "ch")
res_cah_single <-NbClust(x_data[,1:256], distance = "euclidean", min.nc=2, max.nc=10, method = "single",
index = "ch")
res_cah_kmeans <-NbClust(x_data[,1:256], distance = "euclidean", min.nc=2, max.nc=10, method = "kmeans",
index = "ch")
...
# Q5 : HCPC
En observant le graph de l'inertie, on peut remarquer que nous avons un premier coude au niveau de la 2 éme barre ainsi qu'un 2 éme coude au niveau de la 4éme barre, nous pouvons donc proposer 2 ou 4 clusters, 4 se rapprochant plus du nombre de classes original.
```{r}
res.PCA<-PCA(x_data,ncp=2,quali.sup=c(257),graph=FALSE)
...
```{r}
set.seed(1)
res.PCA<-PCA(x_data,ncp=8,quali.sup=c(257),graph=FALSE)
...
```{r}
set.seed(1)
res.mixmod = mixmodCluster(data=as.data.frame(res.PCA$ind$coord),nbCluster=1:10,models=mixmodGaussianModel(listModels = c("Gaussian_p_Lk_Ck", "Gaussian_pk_Lk_Ck", "Gaussian_pk_Lk_Dk_A_Dk", "Gaussian_p_Lk_Dk_A_Dk")))
# 
#res.mixmod
summary(res.mixmod)
...
Les resultats des deux packages sont cette fois-ci tres different, on remarque que à part pour le cluster 8 de mclust qui semble avoir été englobé dans le cluster 1 rmixmod le reste des clusters semblent tres differents.
```{r}
...
# Q11: TSNE
```{r}
tsne_res <- Rtsne(x_data[,1:256], dims = 2, perplexity=30, verbose=TRUE, max_iter = 2500)
```
```{r}
...
```{r}
ae_model <- h2o.deeplearning(x=c(1:256),
                                  model_id="ae_model",
                                  training_frame=as.h2o(x_data[,1:256]),
                                  activation="Tanh",
...
```{r}
features <- h2o.deepfeatures(ae_model, as.h2o(x_data[,1:256]), layer=3)
```
...
```{r}
res.mixmod = mixmodCluster(data=as.data.frame(features), nbCluster=1:10,models=mixmodGaussianModel(listModels = c("Gaussian_p_Lk_Ck", "Gaussian_pk_Lk_Ck", "Gaussian_pk_Lk_Dk_A_Dk", "Gaussian_p_Lk_Dk_A_Dk")))
```
