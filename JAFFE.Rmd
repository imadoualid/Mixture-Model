---
title: "Projet Mixture model : JAFFE"
...
JAFFE est un dataset Contenant des images d'expressions facials de femme japonaises, avec 676 variables, le dataset est repartie en 10 classes.

```{r}
jaffe = readMat(con="C:\\Users\\GIGABYTE\\Downloads\\DATA_MATLAB - Projet-master-MLDS\\DATA_MATLAB - Projet-master-MLDS\\jaffe.mat")
```



```{r}
y_data = as.data.frame(jaffe$y)
x_data = as.data.frame(jaffe$X)
...
# Question 2: ACP
Depuis le graph des individus, on peut remarquer que la plus part des classes sont parfaitement séparable entre elles, cependant il existe un chevauchement important entre certains clusters, notamment y_3 et y_10 ou y_1 et y_7.
Lorsque l'on représente uniquement les modalité de la variable cible y, on peut confirmer notre précedente observation, on remarque aussi que les individu de la classe y_4 contribuent fortement à la creation de l'axe 2, la méme observation peut étre faite pour y_6 et y_3 par rapport à l'axe 1.

```{r}
res.PCA<-PCA(x_data,quali.sup=c(677),graph=FALSE)
plot.PCA(res.PCA,choix='var',title="Graphe des variables de l'ACP")
plot.PCA(res.PCA,invisible=c('quali','ind.sup'),habillage=677,title="Graphe des individus de l'ACP",label ='none')
```
...
res.PCA<-PCA(x_data,quali.sup=c(677),graph=FALSE)
plot.PCA(res.PCA,choix='var',title="Graphe des variables de l'ACP")
plot.PCA(res.PCA,invisible=c('ind','ind.sup'),habillage=677,title="Graphe des individus de l'ACP",label =c('quali'))
```
...
La plus part des algorithme ne trouvent que 4 classes en tant que nombre optimal lorsque nous appliquons un clustering, ce qui est explicable si on compare cela à nos observations precedentes ou nous avions remarquer qu'il existe un chevauchement entre differentes classes dans une meme partition, nous remarquons aussi que l'algorithme CAH average retrouve bel et bien 7 classes.
nous pouvons donc proposer 4 classes si nous nous basons uniquement sur ces rsultats la.

```{r}
res_cah_ward <-NbClust(x_data[,1:676], distance = "euclidean", min.nc=2, max.nc=10, method = "ward.D2",
index = "ch")
res_cah_average<-NbClust(x_data[,1:676], distance = "euclidean", min.nc=2, max.nc=10, method = "average",
index = "ch")
res_cah_complete <-NbClust(x_data[,1:676], distance = "euclidean", min.nc=2, max.nc=10, method = "complete",
index = "ch")
res_cah_single <-NbClust(x_data[,1:676], distance = "euclidean", min.nc=2, max.nc=10, method = "single",
index = "ch")
res_cah_kmeans <-NbClust(x_data[,1:676], distance = "euclidean", min.nc=2, max.nc=10, method = "kmeans",
index = "ch")
...
# Q5 : HCPC
En observant aussi le graph de l'inertie, on peut remarquer un coude à la 3 éme barre, on pourrait aussi aller j'usqu'à la 4 éme barre, il est donc possible de proposer 3 à 4 classes.
```{r}
res.PCA<-PCA(x_data,ncp=Inf, scale.unit=FALSE,quali.sup=c(677),graph=FALSE)
...
Rmixmod trouve aussi 10 classes, cependant le package est beaucoup plus instable, nous avons du restreindre le choix des methodes à essayer, parmis les methodes gardé, la plus efficace est  VEV.
...
```{r}
set.seed(1)
res.PCA<-PCA(x_data,ncp=15,quali.sup=c(677),graph=FALSE)
...
```{r}
set.seed(1)
res.mixmod = mixmodCluster(data=as.data.frame(res.PCA$ind$coord),nbCluster=1:10,models=mixmodGaussianModel(listModels = c( "Gaussian_pk_Lk_Dk_A_Dk", "Gaussian_p_Lk_Dk_A_Dk")))
# 
#res.mixmod
summary(res.mixmod)
...
Les resultats des deux packages sont plus ou moins similaire sur la plus part des clusters (exemple cluster 6 de mclust plus ou moins égale au cluster 8 de rmixmod), nous remarquons cependant un desaccord pour le cluster 2 de mclust avec l'nsembles des clusters de rmixmod
```{r}
...
tsne_res <- Rtsne(x_data[,1:676], dims = 2, perplexity=30, verbose=TRUE, max_iter = 2500)
...
On remarque que mclust obtient de meilleurs resultats de rmixmod.
...
```{r}
ae_model <- h2o.deeplearning(x=c(1:676),
                                  model_id="ae_model",
                                  training_frame=as.h2o(x_data[,1:676]),
                                  activation="Tanh",
...
```{r}
features <- h2o.deepfeatures(ae_model, as.h2o(x_data[,1:676]), layer=3)
```
